...
var logErrMsg = function(file, msg, options) {
...
var finished = function(files, succeedCount, options, fullfill, reject) {
    ...
    // Finished, fullfill or reject promise
    if (succeedCount === files.length) {
        fullfill(succeedCount + ' files passed validation');
    }
    else {
            ...
            if (res === undefined || res.messages === undefined) {
                ...
                for (var i = 0; i < res.messages.length; i += 1) {
                    ...
                    if (!checkRelaxed(res.messages[i].message, options) &&
                        ...
                        errFound = true;
                        logErrMsg(file, res.messages[i], options);
                    }
                }
		...
		return new Promise(function(fullfill, reject) {
			...
			var files = globule.find({
				...
				return {
					...
					attempts: 0,
					wrapping: file.endsWith(options.tmplext) ? options.wrapping : undefined
				}
			});
	        ...
	        async.mapLimit(
	        	...
	        	function(err, results) {
	            	finished(results, results.filter(Boolean).length, options, fullfill, reject);
	        	});
		});
	},
	tester: function() {
