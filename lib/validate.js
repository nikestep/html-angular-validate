...
var writeCheckstyleReport = function(files,options) {
  /*
      // sadly xmlbuilder has a issue with encoding https://github.com/oozcitak/xmlbuilder-js/issues/95
      // so disabling for now this code block, should be enabled as soon the issue gets fixed
  ...
  // hack:begin 
  // sadly xmlbuilder has a issue with encoding https://github.com/oozcitak/xmlbuilder-js/issues/95
  // so needing this bad hack until it is fixed by xmlbuilder and this block can be completely removed
  var str= '<?xml version="1.0" encoding="UTF-8"?>\n';
  str=str.concat('<checkstyle version="'+options.reportCheckstyleVersion+'">\n');
  for (var i = 0; i < files.length; i += 1) {
    var file = files[i];
    var dir = (process.cwd()+"/"+file.path).replace(/\//g, "/");
    var fileStr = '<file name="'+dir+'">\n';
    for (var j = 0; j < file.errs.length; j += 1) {
      var err = file.errs[j];
      ...
      var errorStr = '<error line="'+err.line+'"  col="'+err.col+'" severity="'+options.reportCheckstyleSeverity+'" source="html-angular-validate" message="'+xmlMessage+'"/>\n';
      fileStr = fileStr.concat(errorStr);
    }

    fileStr = fileStr.concat('</file>\n');
    str = str.concat(fileStr);
  }	  

  str=str.concat('</checkstyle>');
